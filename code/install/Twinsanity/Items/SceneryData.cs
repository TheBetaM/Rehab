...
namespace Twinsanity
{
    public class SceneryData : TwinsItem
    {
        ...
        private void CountSceneryModel(SceneryModelStruct ptr, ref int count)
        {
            ...
            count += ptr.LightBitFields.Length * 4;
        }
        ...
        public override void Save(BinaryWriter writer)
        {
            ...
            if ((HeaderUnk1 & 0x20000) != 0)
            {
                ...
                for (int i = 0; i < LightsAmbient.Count; i++)
                {
                    writer.Write(i);
                    writer.Write(0);
                }
                for (int i = 0; i < LightsDirectional.Count; i++)
                {
                    writer.Write(i);
                    writer.Write(1);
                }
                for (int i = 0; i < LightsPoint.Count; i++)
                {
                    writer.Write(i);
                    writer.Write(2);
                }
                for (int i = 0; i < LightsNegative.Count; i++)
                {
                    writer.Write(i);
                    writer.Write(3);
                }
                for (int i = 0; i < 0x400 - (8 * LightsNum); i++)
                {
        ...
        private void SaveSceneryModel(SceneryModelStruct ptr, BinaryWriter writer)
        {
            ...
            for (int i = 0; i < ptr.LightBitFields.Length; i++)
            {
                writer.Write(ptr.LightBitFields[i]);
            }

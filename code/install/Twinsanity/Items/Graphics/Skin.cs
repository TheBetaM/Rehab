            ...
            { 
                ...
                for (int j = 0; j < verts; j++)
                {
                    var vertData = new VertexData
                    {
                        ...
                        // Colors
                        R = (byte)(Math.Min((int)(vertex_batch_4[j].GetBinaryX() & 0xFF), 255)),
                        G = (byte)(Math.Min((int)(vertex_batch_4[j].GetBinaryY() & 0xFF), 255)),
                        B = (byte)(Math.Min((int)(vertex_batch_4[j].GetBinaryZ() & 0xFF), 255)),
                        A = (byte)(Math.Min((int)(vertex_batch_4[j].GetBinaryW() & 0xFF), 255)),
                        Joint = jointInfos[j],
                        Conn = connections[j]
                    };

...
namespace Twinsanity
{
    ...
    {
        private static bool PaletteFix = true; // Double RGB values for nice export (do not use for modding)
        ...
        public override void Load(BinaryReader reader, int size)
        {
            ...
            switch (PixelFormat)
            {
                case TexturePixelFormat.PSMCT32: // Easiest one, raw color data
                    ...
                    var TempData = new Color[Width * Height];
                    var pxInd = 0;
                    for (var i = 0; i < texSize - 224; i += 4)
                    {
                        ...
                        TempData[pxInd++] = Color.FromArgb(a, r, g, b);
                    }
        ...
        public static void SwapPalette(ref Color[] palette)
        {
            for (int i = 0; i < 8; i++)
            {
        ...
        public static void SwapPalette2(ref Color[] palette, int Width, int Height)
        {
            for (int i = 0; i < 8; i++)
            {
                for (int j = 8 + i * 32; j < 16 + i * 32; j++)
